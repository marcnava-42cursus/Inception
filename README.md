...
For evaluation, non-sensitive environment variables must be created locally in `srcs/.env`. Credentials are stored in local Docker secret files under `secrets/` and must not be committed to Git.
...
Create `srcs/.env` locally during evaluation. It should contain configuration values, not passwords.
